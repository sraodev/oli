...
func runCleanup(ctx context.Context, opts cli.Options, engine *cleanup.Engine, home string, output io.Writer) error {
	...
	if opts.Apply {
		...
		result, cleanErr = engine.Clean(ctx, scan, cleanup.CleanOptions{RuleIDs: ruleIDs})
	...
	if opts.JSON {
		...
		if err := writeJSON(output, struct {
			...
			SchemaVersion: schemaVersion, Mode: mode, Profile: profile, RuleIDs: ruleIDs, Scan: scan, Result: result,
		...
		if !opts.Apply {
			...
			printCleanResult(output, result, observed)
			if cleanErr != nil {
	...
	if result != nil && (len(result.Failures) > 0 || len(result.Rejected) > 0) {
		return fmt.Errorf("cleanup finished with %d failed or changed candidates",
			len(result.Failures)+len(result.Rejected))
	}
	return nil
}
...
func printCleanResult(output io.Writer, result *cleanup.CleanResult, observed int64) {
	if result == nil {
		return
	}
